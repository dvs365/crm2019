...
<?php $form = ActiveForm::begin(['action' => ['index', 'sort' => $sort, 'role' => $role], 'method' => 'get', 'options' => ['class' => 'filters wrap4', 'id' => 'searchclient']]); ?>
    <div class="wrap1">
        ...
        <div class="wrap_half">
            <?=$form->field($model, 'task', ['template' => "<label class=\"lh36\">{input}<span class=\"checkbox\"></span> С активными делами</label>"])->checkbox([], false)?>
            <?=$form->field($model, 'disconfirm', ['template' => "<label class=\"lh36\">{input}<span class=\"checkbox\"></span> С несогласованной скидкой</label>"])->checkbox([], false)?>
        </div>
        <div class="clear"></div>
        <?endif;?>
    </div>
