    ...
    xCal.f = function(a) {
        ...
        if (a.o.name == 'date-to') {
            ...
            for(var i=1; i<=Dlast; i++) {
                ...
                else if ((typeof par !== 'undefined') && (par.pform == 'sum') && (Number(isDate) > Number(today))) ca += ' class="disabled"';
                else if(i==dat) {
            ...
            for(var i=1; i<=Dlast; i++) {
                ...
                isDate = String(a.year) + String(isMonth) + String(isDay);
    	        ca += '<td';
                if ((typeof par !== 'undefined') && (par.pform == 'sum') && (Number(isDate) > Number(fD))) ca += ' class="disabled"';
    	        if(i==dat) {
    		        if(m==a.smonth && y==a.syear) ca += ' class="today"';
    		        else ca += ' class="tday"';
    	        } else if(i==a.tdat && m==a.tmonth && y==a.tyear) ca += ' class="tday"';
...
$('html').on('click', 'body', function(e){
 	if ($("table#xcalend").is(':visible')) {
        var div = $("input");
        if (!div.is(e.target) && div.has(e.target).length === 0) {
            $("table#xcalend").hide(0);
        }
    }
});